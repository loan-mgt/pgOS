...
class MainApp(MDApp):
	...
	for x in dirr:
			...
			if x== 'notloged':
					...
					login_satus=[{"text":"se connecter"}]
			else:
					for x in dirr:
							
							if x[0:3]== 'log':
									login_name = x[4:len(x)]
									login_satus = [{"text":"Sauvegarde"}]



	print("login_satus",login_satus,",","login_name",login_name)


	def tread_update(self, df=1):
		id_folder = None
		dirr = os.listdir()
		print("scherhing for :notloged",dirr)
		for x in dirr:
	...
	def set_item_ac(self, instance):
			...
			if self.login_satus==[{"text":"Sauvegarde"}]:
					...
					btn_ac  = Button(text='Faire une Sauvegarde',size_hint= (0.2,0.1),pos_hint={'x': 0.2, 'y': 0.5})
